// Données originales sous forme de texte
const data = 
`
...
function formatData(data) {
  ...
  lines.forEach((line) => {
    ...
    if (match) {
      ...
      const cubesInfo = match[2].split(';').map((set) => set.trim().split(', '));

      // Ajouter les informations du jeu au résultat
      result.push([gameId, ...cubesInfo]);
    }
  });
...
// Utilisation de la fonction formatData pour traiter les données
const jeux = formatData(data);

// Afficher le résultat dans la console
// console.log(jeux);

function trouverJeuxPossibles(jeux) {
  const resultatsPossibles = [];
  const sacAttendu = { rouge: 12, vert: 13, bleu: 14 };

  function sacEstValide(sac) {
    return (
      sac.rouge <= sacAttendu.rouge &&
      sac.vert <= sacAttendu.vert &&
      sac.bleu <= sacAttendu.bleu
    );
  }

  function verifierJeuxPossibles(jeux) {
    for (const jeu of jeux) {
      const sac = { rouge: 0, vert: 0, bleu: 0 };

      for (const ensemble of jeu.slice(1)) {
        const cubes = ensemble.split(', ');

        for (const cube of cubes) {
          const [quantite, couleur] = cube.split(' ');
          sac[couleur] += parseInt(quantite, 10);
        }
      }

      if (sacEstValide(sac)) {
        resultatsPossibles.push(jeu);
      }
    }
  }

  verifierJeuxPossibles(jeux);

  return resultatsPossibles;
}

// Utilisation de la fonction avec les sacs attendus
const resultats = trouverJeuxPossibles(jeux);

// Calcul de la somme des identifiants des jeux possibles
const sommeIdentifiants = resultats.reduce((acc, jeu, index) => acc + (index + 1), 0);

console.log("Jeux possibles :", resultats);
console.log("Somme des identifiants des jeux possibles :", sommeIdentifiants);
